...
1. `applyMultiSpecTemplateVars` runs at the end of the merge. It unions `EndpointTemplateVars` (first declaration wins), `EndpointTemplateEnvOverrides` (first wins; a conflicting later name warns on stderr naming both specs), and the two default maps through a shared `mergeFirstWins` helper, then re-runs `DropCollidingEndpointTemplateEnvOverrides` because the merged CLI has its own name and auth model. It sets `GlobalPathTemplateVars` to nil so the generator re-derives globals from the merged endpoints.
2. `compatibleOAuthScopeAuth` checks type and effective grant first, then treats `client_credentials` as compatible when normalized token URLs and refresh-token mechanisms match; the authorization-URL requirement still governs every other grant.

Proof lives in `internal/cli/generate_test.go`: merge table tests for each union and conflict case, plus two generated-output tests that generate from two specs, assert the emitted `--tenant` flag and scope literal, and compile the module.
